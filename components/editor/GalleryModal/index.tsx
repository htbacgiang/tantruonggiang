...
const GalleryModal: FC<Props> = ({
  ...
  // Sử dụng object để lưu trữ alt text cho từng hình ảnh
  ...
  const handleSelection = () => {
    ...
    onSelect({ src: selectedImage, altText: altTexts[selectedImage] || "" });
    handleClose();
  };

  // Cập nhật khi chọn hình ảnh mới
  const handleImageSelect = (src: string) => {
    setSelectedImage(src);
  };

  // Cập nhật alt text cho hình ảnh hiện tại
  const handleAltTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (!selectedImage) return;
    const newAltText = e.target.value;
    setAltTexts((prev) => ({
      ...prev,
      [selectedImage]: newAltText,
    }));
  };
